...
public class BezierFollow : MonoBehaviour
{
    ...
    private void WhileDragging()
    {
        ...
        // like kind of sensitivity
            var distCovered = ((Input.GetAxis("Mouse X") * -1) * curveSpeed / _journeyLength)+updated; //Problem: +updated to offsetPos.x. mainly offsetPosX being added profusely SOLVED
        updated = distCovered;
        if(updated >= 1f)
        {
            updated = 1f;
    ...
    private void Update()
    {
        ...
        if((!_isObjectSelected) && Input.GetMouseButtonDown(0))
                {
